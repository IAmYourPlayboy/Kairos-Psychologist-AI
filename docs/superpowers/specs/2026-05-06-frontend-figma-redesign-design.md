...
Порт логики из Figma `AppContext.tsx::useThemeTokens` 1-в-1 в `lib/theme-tokens.ts`. Это просто маппинг булевого флага `isDark` на готовые Tailwind-классы:
...
│   ├── theme-tokens.ts               [NEW] — useThemeTokens
