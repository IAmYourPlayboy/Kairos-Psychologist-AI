 ...
 *
 * Вызывается при каждом рендере, использует `Date.now()`. Если пользователь
 * держит вкладку открытой в полночь — совет переключится на следующий.
 * Это OK: не считаем нужным фиксировать совет на сессию.
 */
