...
### 1.5.8 Dev-панель (только агрегаты) ⚪

Система-центричная, без доступа к индивидуальным диалогам чужих пользователей. Решение Сессии 25: читать чужую боль = нарушить доверие, на котором стоит продукт.

**Acceptance:**
- [ ] Страница `/dev/dashboard` — доступна только для `User.is_dev_tester=True` (добавить булевое поле в модель User, миграция)
- [ ] Метрики на дашборде:
  - **Здоровье системы**: Redis up/down, PostgreSQL connections, Celery workers status, latency p50/p95/p99 за последний час
  - **Трафик**: активные сессии сейчас, сообщения/мин, tokens/мин, ₽/день (расчётная стоимость Qwen)
...
- [ ] Исключение для **твоего dev-аккаунта** (`is_dev_tester=True`): страница `/dev/my-state` показывает **твой собственный** `UserState`, trace последнего сообщения (собранный промпт, report, выбранная техника). Это безопасно — ты видишь только себя. Для остальных эта страница 404.
- [ ] Ничего из этого не экспортируется наружу (не Sentry, не Grafana — только локальный browser-view)
- [ ] Тесты: non-dev-tester → 403 на обе страницы, dev-tester → видит агрегаты и свой state
